...
for (var j = 0; j < lineLength; j++)
{
	...
	{
		arrayOfOneRow.push(inputArray.pop());
	}
	for (var m = 0; m < arrayOfOneRow.length; m++)
	{
		arrayOfOneRow[m] = +arrayOfOneRow[m];
	}

	objectOfArrays[j] = arrayOfOneRow;
}

for (var k in objectOfArrays)
{
	var thisRow = objectOfArrays[k];
	var largestInRow = thisRow[0];
	var smallestInRow = thisRow[0];

	for (var l = 0, len = thisRow.length; l < len; l++)
	{
		if (thisRow[l] > largestInRow)
		{
			largestInRow = thisRow[l];
		}

		if (thisRow[l] < smallestInRow)
		{
			smallestInRow = thisRow[l];
		}
	}
